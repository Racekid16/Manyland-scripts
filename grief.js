        ...
        if (i % 2 == 0) {
            ...
            for (let j = edgeArrOrganizedByY[i][1][0]; j <= edgeArrOrganizedByY[i][1][edgeArrOrganizedByY[i][1].length - 1]; j++) {
                ...
                if (!tired) {
                    ...
                    await delay(moveWait);
                    if (Math.random() < 0.3 && deleteHistory.length > 2) {
                        ig.game[map][place]("54f0fc2d8a45c059367567b9", 0, 0, {x: deleteHistory[deleteHistory.length - 2][0], y: edgeArrOrganizedByY[deleteHistory[deleteHistory.length - 2][1]][0]}, null, !0);
                    }
                } else {
                    if (!alreadyStopped) {
